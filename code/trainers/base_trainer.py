...
class PolicyMethodMixin:
    ...
    def get_max_reward(self, config: TrainingConfig) -> float:
        """Estimate maximum possible reward used for NSR's threshold."""
        # Chart-RVR base components, summed at their per-component ceilings:
        # format(2) + accuracy(1) + length(2) + token-count(2) +
        # chart-type(1) + table(2) + process(1) = 11
        max_base = 11.0

        # HCPC ceiling: (|G+|/K) * (w_table + w_reason) is bounded by
        # (w_table + w_reason) when all K rollouts are correct.
        max_hcpc = 0.0
        if config.rewards.use_hcpc:
            max_hcpc = config.rewards.w_table + config.rewards.w_reason

        return max_base + max_hcpc
