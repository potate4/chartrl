...
flowchart LR
    ...
    C --> C2["CLC reward → NOT yet enabled\n(planned for next run)"]
```
...
| D_reason | 0.0400 | **0.0579** | 0.0521 |
| Coherence | **0.5452** | 0.2516 | 0.2444 |
...
> D_reason values multiplied by 10 for visibility; actual values: Base=0.040, GRPO=0.058, GRPO-HCPC=0.052.
...
### 6.2 D_reason: A Critical Anomaly

The `d_reason` metric (reasoning diversity across correct rollouts) tells a counterintuitive story:

| Model | D_reason (all samples) | D_reason (correct-only samples) |
|---|---:|---:|
| Base | 0.0400 | 0.0517 |
| GRPO | **0.0579** | **0.0709** |
| GRPO-HCPC | 0.0521 | 0.0620 |

GRPO-HCPC has **lower** diversity than GRPO baseline on both measures. This directly contradicts the intended HCPC objective of encouraging reasoning diversity. Several explanations are possible:

1. **HCPC weight imbalance**: `w_reason` may be too low relative to `w_table` and `w_type`. The table consistency and chart-type components dominate, resulting in a net convergent pressure.
2. **Too few correct rollouts during training**: HCPC requires ≥2 correct rollouts per group to activate the diversity signal. If training accuracy was low in early epochs, the diversity reward rarely fired.
3. **CLC is absent**: The Cross-Level Coherence reward (which penalises rollouts that don't reference extracted values) was not enabled. Without it, the model has no incentive to maintain grounded reasoning, which may actually reduce apparent diversity since more rollouts resort to short template answers.
4. **Evaluation rollout count**: Only 4 rollouts are used at evaluation. Diversity signals are more stable with 8 or more rollouts.

```mermaid
flowchart TD
    A["D_reason(GRPO-HCPC) < D_reason(GRPO)\nContradicts HCPC design goal"]
    A --> B{Root cause?}
    B --> C["w_reason too low\nvs w_table, w_type"]
    B --> D["HCPC rarely fires\nduring training\n(needs ≥2 correct rollouts)"]
    B --> E["CLC absent\nNo grounding incentive"]
    B --> F["4 rollouts insufficient\nfor diversity estimate"]
```

### 6.3 Coherence Drop

Coherence fell sharply from the base model (0.545) to both trained models (GRPO: 0.252, GRPO-HCPC: 0.244). This metric measures whether reasoning text explicitly reuses numeric values from the extracted table.

This is most likely a **style shift** rather than capability loss. Both trained models:
- Adopt shorter, more direct reasoning traces under reward pressure
- Are rewarded for correct final answers regardless of how verbosely they cite intermediate values
- Have no CLC reward incentivising table-grounded reasoning

The practical implication for the thesis: CLC is the natural complement to HCPC. Without CLC, both models converge toward concise templates that score well on accuracy but may hallucinate intermediate steps. The coherence drop is a symptom of the absent CLC component.

### 6.4 Format Compliance Breakthrough
...
The base model only produces correctly structured output 29.8% of the time. Both RL-trained models reach ~97-98%. This reflects the format reward doing its job and is a prerequisite for all downstream structural metrics (c_table, d_reason, coherence) to be meaningful.
...
flowchart LR
    ...
    subgraph CONCERN["Open concern"]
        C1["D_reason unexpectedly lower\nthan GRPO — contradicts\nHCPC design intent"]
        C2["Coherence dropped for\nboth trained models\n— CLC not yet active"]
        C3["No OOD evaluation\nEvoChart results missing"]
    end
```
...
3. **HCPC preserves accuracy while changing the training objective.** The 0.8 pp accuracy gap between GRPO (0.630) and GRPO-HCPC (0.622) is small. Switching from a process-reward objective to a group-level consistency objective does not meaningfully damage single-answer performance.

4. **HCPC is faster.** The trained GRPO-HCPC model generates at base-model speed. This is a secondary but practically relevant advantage.

### 8.2 What the Results Do Not Yet Prove

1. **Reasoning diversity is not demonstrated.** D_reason is lower for GRPO-HCPC than GRPO. The diversity component of HCPC is not working as designed.

2. **OOD generalisation is unverified.** The strongest thesis argument — that HCPC reduces the train/test distribution gap — cannot be made without EvoChart results.

3. **CLC has not been tested.** The Cross-Level Coherence reward is part of the full HCPC-RLVR design but was not enabled. Its contribution to coherence recovery and hallucination prevention is unknown.

4. **NSR and W-REINFORCE have not been run.** Four of the six planned experiments remain untested. The hypothesis that NSR or W-REINFORCE combined with HCPC further improves diversity is unverified.
...
### Priority 2 — Fix D_reason: Adjust HCPC Weights

The D_reason anomaly must be investigated before running more experiments. Suggested changes:

- Increase `w_reason` (diversity weight) relative to `w_table` and `w_type`
- Log the per-step fraction of training batches where HCPC fires (needs ≥2 correct rollouts)
- Consider lowering the correctness threshold for HCPC to fire more frequently early in training

### Priority 3 — Enable CLC

Add the Cross-Level Coherence reward to the next GRPO-HCPC run. This should:
- Partially recover coherence scores
- Penalise hallucinated reasoning (correct answer, fabricated steps)
- Complete the full HCPC-RLVR design described in the thesis

### Priority 4 — Run NSR and W-REINFORCE Experiments
...
NSR is particularly promising: it only penalises wrong samples rather than reinforcing a single correct path. This should naturally raise D_reason without needing heavy reward engineering.

### Priority 5 — Scale Training
...
> GRPO-HCPC training produces models with significantly more reliable rollout sets. On ChartQA, 44.4% of questions are solved correctly across all 4 sampled rollouts, compared to 37.2% for GRPO and 23.4% for the base model. This improvement in rollout-level reliability — captured by pass@k and correct_rate — demonstrates that the group-level HCPC objective shapes the distribution of candidate responses in a practically beneficial direction.
