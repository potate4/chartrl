...
\begin{abstract}
Reinforcement learning recipes from text-only math RLVR are
increasingly being adapted to multimodal training pipelines. We
investigate one such recipe---Negative-Sample Reinforcement (NSR;
\citealp{zhu2025nsr}), which masks the gradient of correct
rollouts---together with \textbf{HCPC}, a cross-rollout bonus we
propose that rewards consistent extraction and diverse reasoning
among ground-truth-correct rollouts. We report two main findings
and one supporting one. \textbf{(1)} NSR ported unchanged collapses
format compliance in chart RLVR
($96.8\%\!\to\!31.6\%$ on ChartQA,
...
unprompted, with a mechanistic explanation (structural rewards
saturate, well-formed rollouts cross NSR's threshold, NSR masks
their gradient). \textbf{(2)} Adding HCPC to NSR closes most of the
OOD accuracy gap to GRPO (NSR+HCPC Pass@1 $0.632$ vs.\ $0.638$) at
near-zero format compliance ($17.8\%$ vs.\ $98.8\%$): the
conditional gap
$P(\text{correct}\mid\text{formatted}){-}P(\text{correct}\mid\text{not formatted})$
drops from $0.30$ under GRPO to $0.05$ under NSR+HCPC, empirical
evidence that the structured-output scaffold is no longer
load-bearing in the answer pathway. The recovery operates through a
baseline-shift mechanism in the GRPO advantage computation, not
direct positive reinforcement on correct rollouts.
\textbf{(3, supporting)} On top of GRPO alone, HCPC matches the
baseline on Pass@1 ($0.622$ vs.\ $0.630$) and trends ahead on
rollout-set robustness ($40.8\%$ all-4-correct vs.\ $38.6\%$, Pass@4
$0.828$ vs.\ $0.816$), consistent with the cross-rollout signal
...
increasingly being adapted to multimodal training pipelines. Whether
those recipes transfer cleanly is a meaningful empirical question:
multimodal RLVR pipelines---particularly for chart reasoning
\citep{chartrvr2025,chartr12025,bigcharts2025,distill-vcr-2025}---
typically use a \emph{multi-component} reward landscape (format,
chart type, table reconstruction, process conformity, answer
accuracy) very different from the single answer bit that drives
text-only math RLVR. An advantage-rule trick tuned for one might
behave very differently under the other.

We study this question concretely by porting Negative-Sample
Reinforcement \citep[NSR;][]{zhu2025nsr}---a recipe from recent
text-only math RLVR that masks the gradient of correct rollouts and
penalizes only incorrect ones---into a chart-reasoning RLVR pipeline
built on Chart-RVR \citep{chartrvr2025}. To exercise the reward
landscape we also propose \textbf{HCPC} (Hierarchical Correct-Path
Consistency), a cross-rollout bonus that conditions on ground-truth
correctness and rewards consistent table extraction together with
diverse reasoning traces, applied per-rollout to the correct-path
subgroup. HCPC enters the paper both as a positive-reinforcement
augmentation in its own right (we report its effects on top of GRPO)
and as the cross-rollout signal that, when combined with NSR,
exposes a clean dissociation between two behaviors of the trained
policy that are intuitively coupled.

\paragraph{Contributions.}

\begin{enumerate}
    \item \textbf{An NSR-on-VLM transferability finding
    (\S\ref{sec:nsr_collapse}).} Porting NSR unchanged from
    text-only math collapses format compliance in chart RLVR
    ...
    \item \textbf{HCPC mitigates NSR's OOD damage and exposes a
    format/accuracy decoupling (\S\ref{sec:dissociation}).} Adding
    HCPC to NSR closes most of the OOD accuracy gap to GRPO
    (Pass@1 $0.632$ vs.\ $0.638$) at \emph{near-zero} format
    compliance ($17.8\%$ vs.\ $98.8\%$). Per-rollout analysis shows
    the conditional gap
    $P(\text{correct}\mid\text{formatted}){-}P(\text{correct}\mid\text{not formatted})$
    drops from $0.30$ under GRPO to $0.05$ under NSR+HCPC: the
    structured-output scaffold has become non-load-bearing in the
    answer pathway. The recovery operates through a baseline shift
    in the GRPO advantage---HCPC's bonus on correct rollouts raises
    the group mean, making wrong-rollout advantages more negative
    under NSR's mask---rather than direct positive reinforcement on
    correct rollouts.
    \item \textbf{HCPC as a positive-reinforcement augmentation on
    GRPO (\S\ref{sec:hcpc_grpo}).} On top of standard GRPO, HCPC
    matches the Chart-RVR baseline on Pass@1
    ($0.622$ vs.\ $0.630$, ns) and produces more robust correct-path
    behavior: $40.8\%$ all-4-correct rate vs.\ $38.6\%$, Pass@4
    $0.828$ vs.\ $0.816$, and table-dependence
    $\Delta_{\text{tbl}}{=}{+}0.264$ vs.\ ${+}0.178$. Individual
    differences are within paired-bootstrap noise at $n{=}500$, but
    all in the direction the cross-rollout signal is designed to
    produce. This contribution is auxiliary: the strongest evidence
    in the paper is in (1) and (2), and we report (3) as the
    in-distribution behavioral signature of the same signal that
    drives the dissociation.
\end{enumerate}
...
\paragraph{Format/accuracy decoupling.} The way in which HCPC
...
GRPO's $98.8\%$). To check whether this is a genuine decoupling or
a denominator artifact, we compute the per-rollout conditional
probability that an answer is correct given that the rollout is
fully formatted, vs.\ given that it is not:
\[
\Delta_{\text{fmt}} = P(\hat y=y^*\mid\text{fmt}) - P(\hat y=y^*\mid\text{\textasciitilde fmt}).
\]
Under GRPO, $\Delta_{\text{fmt}} = 0.664 - 0.368 = 0.296$:
formatted rollouts are dramatically more likely to be correct,
indicating that the structured-output scaffold is load-bearing in
GRPO's answer pathway. Under NSR+HCPC, $\Delta_{\text{fmt}} =
0.661 - 0.608 = 0.053$: formatted and unformatted rollouts answer
correctly at nearly the same rate. NSR+HCPC has effectively
decoupled answer accuracy from the structured-output scaffold:
unformatted rollouts (the majority, 91.3\%) still reach correct
answers at $60.8\%$, well above any plausible guessing rate on this
benchmark.\footnote{ChartFC is a fact-checking task with a
constrained answer space, which raises the floor; we report the
conditional gap as the apples-to-apples decoupling measure.}
...
OOD---is what makes the decoupling claim concrete.

\subsection{HCPC under GRPO: Auxiliary In-Distribution Evidence}
\label{sec:hcpc_grpo}

We also report HCPC's behavior as a standalone positive-reinforcement
augmentation on top of standard GRPO, as in-distribution evidence
that the cross-rollout bonus is behaving as designed (rather than
purely as an instrument for revealing the NSR dissociation).
...
table-dependence is $\Delta_{\text{tbl}}{=}{+}0.264$ vs.\ $+0.178$;
format compliance is $98.2\%$ vs.\ $96.8\%$. No individual difference
reaches significance at $n{=}500$ (Pass@4 boot-$p{=}0.48$), but the
\emph{joint} direction across six metrics in HCPC's favor is
...
Unpacking $\Delta_{\text{tbl}}$ for HCPC under GRPO:
$P(\hat y=y^*\mid\text{tbl})$ is $0.639$ for GRPO and $0.632$ for
HCPC (tied; also tied on the common-table intersection of 462
samples, Appendix~C), while $P(\hat y=y^*\mid\text{no tbl})$ is
$0.462$ for GRPO and $0.368$ for HCPC. HCPC's $\Delta_{\text{tbl}}$
gain therefore reflects stronger \emph{table-dependence}---the
extracted table becomes more load-bearing in the answer pathway---
which is the in-distribution mirror of the decoupling effect we see
under NSR+HCPC. The same cross-rollout signal that, paired with
NSR, breaks the answer pathway's reliance on format-structured
output produces, paired with GRPO, a model whose answer correctness
is more strongly tied to its own extracted table.

\paragraph{GRPO+HCPC OOD limitation.} This same property makes
GRPO+HCPC \emph{worse} than plain GRPO on the OOD ChartFC probe:
...
format compliance drops from $98.2\%$ to $66.4\%$ on ChartFC,
indicating the extraction pipeline is less reliable on unfamiliar
visual styles), the answer pathway degrades with it. The
table-dependence that benefits HCPC in-distribution becomes a
liability OOD. This is a genuine limitation of standalone
GRPO+HCPC as a method, complementary to the NSR-mitigation result
in \S\ref{sec:dissociation}; we leave a fix (e.g., a chart-style
augmentation in training or a softer GT-anchored filter) to future
work.
...
Porting Negative-Sample Reinforcement from text-only math RLVR into
multimodal chart RLVR collapses format compliance ($96.8\%\!\to\!31.6\%$
on ChartQA) and significantly hurts OOD Pass@4, for a mechanically
identifiable reason: Chart-RVR's structural rewards saturate quickly
enough that well-formed rollouts cross NSR's correctness threshold
regardless of answer quality, and NSR masks their gradient. Adding
HCPC, a cross-rollout bonus we propose, on top of NSR recovers
GRPO-level OOD answer accuracy at near-zero format compliance:
formatted and unformatted rollouts under NSR+HCPC reach correct
answers at nearly the same rate
($P(\text{correct}\mid\text{fmt}){-}P(\text{correct}\mid\text{\textasciitilde fmt}){=}0.05$
under NSR+HCPC vs.\ $0.30$ under GRPO), evidence that the
structured-output scaffold has become non-load-bearing in the
answer pathway. The recovery operates through a baseline-shift
mechanism in the GRPO advantage---HCPC's bonus on correct rollouts
raises the group mean, making wrong-rollout advantages more negative
under NSR's mask---not through direct positive reinforcement. As an
in-distribution check, GRPO+HCPC matches the Chart-RVR baseline on
Pass@1 and trends ahead on rollout-set robustness. The takeaway for
