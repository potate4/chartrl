...
identification, table reconstruction, and answer accuracy. We
investigate two interventions on top of this pipeline: applying an
asymmetric advantage rule (Negative-Sample Reinforcement, NSR;
\citealp{zhu2025nsr}) that masks the gradient of correct rollouts
and penalizes only incorrect ones, and adding \textbf{HCPC}, a
cross-rollout bonus that conditions on ground-truth correctness and
rewards consistent extraction together with diverse reasoning. On
Qwen2.5-VL-3B, NSR ported unchanged collapses format compliance
...
separable reward channels in chart RLVR; HCPC's recovery operates
through a baseline shift in the GRPO advantage that provides no
direct positive gradient on correct rollouts.
\end{abstract}
